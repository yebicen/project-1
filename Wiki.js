$(document).ready(function(){
    ...
    function displayWikipedia() {
     ...
     $.ajax({
         ...
         //need to test min(4,JSON.parse(response).docs.length)
         var n = 0;
         for (var i = 0; i < min(4,JSON.parse(response).docs.length); i++) {
             n=i+1;   
     ...
     .done(function(data) {
       ...
       for (var i = 0; i < 5; i++) {
         ...
         $('<button type="button" class="btn btn-dark save">').text("Save").appendTo("#doc"+y);
